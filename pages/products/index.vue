      ...
      <v-row dense>
        <v-col md="3">
          <div style="position: sticky; top: 92px">
            <v-text-field prepend-inner-icon="mdi-magnify" outlined v-model="search" clearable placeholder="Search"></v-text-field>
        ...
        <v-col md="9">
          <v-row>
            <template>
              <v-fade-transition>
                <v-col cols="12" md="6">
                  <v-card nuxt v-for="(p, i) in filteredProducts" :key="`product${p.id}-${i}`" :to="`/products/${p.id}`" color="surface" class="el ma-2 mb-5 mr-5">
                    ...
                    <v-card-subtitle class="primary--text pb-3"> ${{ p.price }} </v-card-subtitle>
                    <v-card-text>
